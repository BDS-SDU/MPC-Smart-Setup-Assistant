...
def _build_default_program(req: ParsedRequirement) -> str:
    ...
    if req.operation == "comparison":
        return """from Compiler.types import sint
...
class MpSpdzRunner:
    def run(
        ...
        if suffix == ".sh":
            ...
            run_cmd = [bash_path, script_arg, program_name, "-N", str(party_count)]
        else:
            run_cmd = [str(script_path), program_name, "-N", str(party_count)]
