...
class OpenSourceCatalogTests(unittest.TestCase):
    ...
    def test_semi2k_catalog_contains_mp_spdz_spu_and_crypten(self) -> None:
        req = parse_requirement(
            {
        ...
        result = recommend_open_source_protocols(req, limit=20)
        implementation_ids = {item["implementation_id"] for item in result["matches"]}

        self.assertEqual(result["matches"][0]["implementation_id"], "mp_spdz_semi2k")
        self.assertIn("mp_spdz_semi2k", implementation_ids)
        self.assertIn("spu_semi2k", implementation_ids)
        self.assertIn("crypten_semi2k", implementation_ids)
