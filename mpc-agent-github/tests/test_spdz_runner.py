...
class SpdzRunnerUtilsTests(unittest.TestCase):
    ...
    def test_default_comparison_program_allows_binary_sint_injection(self) -> None:
        ...
        source = _build_default_program(req)

        self.assertNotIn("from Compiler.types import sint", source)
        self.assertIn("sint.get_input_from(0)", source)
...
class SpdzRunnerFlowTests(unittest.TestCase):
    ...
    def test_run_omits_party_count_for_yao_script(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "Programs" / "Source").mkdir(parents=True, exist_ok=True)
            (root / "Scripts").mkdir(parents=True, exist_ok=True)
            (root / "Scripts" / "yao.sh").write_text("#!/bin/sh\n", encoding="utf-8")
            (root / "compile.py").write_text("print('compiled')\n", encoding="utf-8")
            (root / "yao-party.x").write_text("", encoding="utf-8")
            ...
            config = FinalConfiguration(
                ...
                title="test",
                mpspdz_home=str(root),
                script_candidates=["Scripts/yao.sh"],
                parties=2,
                security_model="semi_honest",
                corruption_model="dishonest_majority",
                circuit_domain="boolean",
            ...
            try:
                result = MpSpdzRunner().run(
                    {"execute": True, "mpspdz_home": str(root), "program_name": "demo_yao"},
                    req,
                    config,
                )
            finally:
            ...
            self.assertGreaterEqual(len(seen_commands), 2)
            run_cmd = seen_commands[-1]
            self.assertNotIn("-N", run_cmd)
            self.assertEqual(run_cmd[-1], "demo_yao")
